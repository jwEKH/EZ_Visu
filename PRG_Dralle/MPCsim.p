...
/* MPCSIM: Simulation der Ein- Ausgaenge                             */
...
! MONPROT:  LD/3,0/.MONPXX <->;    /* H0-Dateien fuer Langzeitmeldeprotokoll */
  ...
  BTASTIN:  B2.(NE)       <-> ;    /* Eingang Bedienung Tastatureingabe */
...
/* CAN- Vereinbarungen                                               */
  ...
  DCL AIWERT(250)  FLOAT;
  DCL DIWERT(200)  BIT(1);
  DCL DOWERT(200)  FIXED;
  DCL DOMERK(200)  FIXED;
  DCL AOMERK(200)  FLOAT;

/*-------------------------------------------------------------------*/
#INCLUDE c:\p907\033bgadrallehohne\spc.p;


SENDTEIL: TASK PRIO 10; /* SSSSS */
  DCL FIX1 FIXED;

  REPEAT
    /* Kommunikation f¸r Simulation */
    IF Z_LZ > 5(31) THEN
      FIX1=1;
      FOR I TO N_RELPLT REPEAT
        FOR J TO 8 REPEAT
          IF BI_DAUS(I).BIT(17-J) AND BI_OFF(I).BIT(17-J) OR BI_ON(I).BIT(17-J) THEN
            DOWERT(FIX1)=1000;
          ELSE
            DOWERT(FIX1)=0;
          FIN;
    ...
    ! DOWERT( 7)=ROUND(FL_PWMPRO(21)*10.0);
    ! DOWERT( 9)=ROUND(FL_PWMPRO(23)*10.0);
    ! DOWERT(10)=ROUND(FL_PWMPRO(24)*10.0);
    ! DOWERT(11)=ROUND(FL_PWMPRO(22)*10.0);
    ! DOWERT(16)=ROUND(FL_PWMPRO(25)*10.0);
    ! DOWERT(17)=ROUND(FL_PWMPRO(26)*10.0);
    ! DOWERT(18)=ROUND(FL_PWMPRO(27)*10.0);
    
      FOR I TO N_RELPLT*8 REPEAT
        IF DOWERT(I) /= DOMERK(I) THEN
          PUT TOCHAR(255),'DOE',I,DOWERT(I),TOCHAR(254) TO A1 BY A,A,F(3),F(4),A;
          DOMERK(I)=DOWERT(I);
          AFTER 0.010 SEC RESUME;
        FIN;
      END;  
      FOR I TO N_ANALOG REPEAT
        IF X_AAUS(I) /= AOMERK(I) THEN
          FIX1=ROUND(X_AAUS(I)*10.0)+1000;    
          PUT TOCHAR(255),'AOE',I,FIX1,TOCHAR(254) TO A1 BY A,A,F(3),F(4),A;
          AOMERK(I)=X_AAUS(I);
          AFTER 0.010 SEC RESUME;
        FIN;
      END;  
...
SENDALL: TASK PRIO 10;
  DCL FIX1 FIXED;

  FOR I TO N_RELPLT*8 REPEAT
    IF B_FERN THEN      
      PUT TOCHAR(255),'DOE',I,DOWERT(I),TOCHAR(254) TO LCD BY A,A,F(3),F(4),A;
    ELSE
      PUT TOCHAR(255),'DOE',I,DOWERT(I),TOCHAR(254) TO A1 BY A,A,F(3),F(4),A;
    FIN;
    DOMERK(I)=DOWERT(I);
  ...
  FOR I TO N_ANALOG REPEAT
    FIX1=ROUND(X_AAUS(I)*10.0)+1000;    
    IF B_FERN THEN
      PUT TOCHAR(255),'AOE',I,FIX1,TOCHAR(254) TO LCD BY A,A,F(3),F(4),A;
    ELSE
      PUT TOCHAR(255),'AOE',I,FIX1,TOCHAR(254) TO A1 BY A,A,F(3),F(4),A;
    FIN;
    AOMERK(I)=X_AAUS(I);
...
INITSIM: TASK PRIO 10; /* SSSSS */
  DCL KUERZEL CHAR(3);
  DCL F31     FIXED(31);
  DCL FIX1    FIXED;

  N_FUEHLER=N_FUEHLER+2; /* BHKW VL+RL */

  FOR I TO N_FUEHLER REPEAT
    CASE FP_TYP(I)
      ALT /*  1: Temperatur    */ AIWERT(I)=20.0;
      ALT /*  2: Leistung      */ AIWERT(I)= 0.0;
      ALT /*  3: Temperatur    */ AIWERT(I)=19.0;
      ALT /*  4: %             */ AIWERT(I)= 0.4;
      ALT /*  5: Druck 0- 4 b  */ AIWERT(I)= 2.3;
      ALT /*  6: Druck 0- 6 b  */ AIWERT(I)= 4.3;
      ALT /*  7: Gassensor 0-5V*/ AIWERT(I)=-0.8;
      ALT /*  8: Thermoel      */ AIWERT(I)= 59.0;
      ALT /*  9: Spannung0-30  */ AIWERT(I)= 13.5;
      ALT /* 10: Leist. Seidel */ AIWERT(I)=  8.1;
      ALT /* 11: Raumsoll 15-30*/ AIWERT(I)= 16.6;
      ALT /* 12: PT 500 0-100  */ AIWERT(I)= 23.3;
      ALT /* 13: T 4-20 0-100  */ AIWERT(I)= 24.4;
      ALT /* 14: Df4-20 0-100  */ AIWERT(I)=  5.9;
      ALT /* 15: 0-100 Grad    */ AIWERT(I)=19.1;                 
      ALT /* 16: 0-100 Grad    */ AIWERT(I)=18.1;                 
      ALT /* 17: 0-100 Grad    */ AIWERT(I)=17.1;                 
      OUT /* Fehler        */ AIWERT(I)=-999.0;
    FIN;
    F31=ROUND(AIWERT(I)*10)+1000(31);

    CASE FP_TYP((I)) /* Einheit des F}hlers vorbesetzen:   */
      ALT /*  1 */ KUERZEL='Cel';
      ALT /*  2 */ KUERZEL=' KW';
      ALT /*  3 */ KUERZEL='Cel';
      ALT /*  4 */ KUERZEL='  %';
    ...
    PUT TOCHAR(255),'AI',I,FP_NAME(I),F31,KUERZEL,TOCHAR(254) TO A1 BY A,A,F(3),A,F(6),A;
    AFTER 0.010 SEC RESUME;
  END;

! PUT TOCHAR(255),'AI',35,'Pe Bedarf     ',1125(31),' KW',TOCHAR(254) TO A1 BY A,A,F(3),A,F(6),A;   AIWERT(35)=12.5;
! AFTER 0.055 SEC RESUME;
! PUT TOCHAR(255),'AI',36,'VL Soll Unt   ',1625(31),'Cel',TOCHAR(254) TO A1 BY A,A,F(3),A,F(6),A;   AIWERT(36)=62.5;
! AFTER 0.055 SEC RESUME;
! PUT TOCHAR(255),'AI',37,'VL Ist Unt    ',1525(31),'Cel',TOCHAR(254) TO A1 BY A,A,F(3),A,F(6),A;   AIWERT(37)=52.5;
! AFTER 0.055 SEC RESUME;
! PUT TOCHAR(255),'AI',38,'Pu oben Unt   ',1515(31),'Cel',TOCHAR(254) TO A1 BY A,A,F(3),A,F(6),A;   AIWERT(38)=51.5;
! AFTER 0.055 SEC RESUME;
! PUT TOCHAR(255),'AI',39,'Pu Mitte Unt  ',1415(31),'Cel',TOCHAR(254) TO A1 BY A,A,F(3),A,F(6),A;   AIWERT(39)=41.5;
! AFTER 0.055 SEC RESUME;

  AFTER 0.010 SEC RESUME;
  PUT 'AIEND' TO A1 BY A;
  AFTER 0.010 SEC RESUME;
...
!   FIN;  
!   DIWERT( 7)='1'B;
!   IF I==31 THEN  /* mss kette         */
!     FIX1=1;
!   FIN;  
!   DIWERT(31)='1'B;
!   IF I==14 THEN  /* stoe wwspeisep    */
!     FIX1=1;
!   FIN;  
!   DIWERT(14)='1'B;
!   IF I==20 THEN  /* stoe laubenhzg    */
!     FIX1=1;
!   FIN;  
!   DIWERT(20)='1'B;
!   IF I==21 THEN  /* stoe hebeanlage   */
!     FIX1=1;
!   FIN;  
!   DIWERT(21)='1'B;
!   IF I==22 THEN  /* stoe pvanlage     */
!     FIX1=1;
!   FIN;  
!   DIWERT(22)='1'B;

!   IF I==2 THEN  /* Warn Wasserdruck           */
!     FIX1=1;
!   FIN;  
!   DIWERT(2)='1'B;
!   IF I==8 THEN  /* Pumpe HK2                  */
!     FIX1=1;
!   FIN;  
!   DIWERT(8)='1'B;
!   IF I==9 THEN  /* Pumpe HK3                  */ 
!     FIX1=1;
!   FIN;  
!   DIWERT(9)='1'B;
    PUT TOCHAR(255),'DII',I,DI_NAME(I),FIX1,TOCHAR(254) TO A1 BY A,A,F(3),A,F(1),A;
    AFTER 0.010 SEC RESUME;
  END;  
  PUT TOCHAR(255),'DII',90,'BHKW1 BETRIEB            ',0,TOCHAR(254) TO A1 BY A,A,F(3),A,F(1),A;
  PUT TOCHAR(255),'DII',91,'BHKW1 STOERUNG           ',0,TOCHAR(254) TO A1 BY A,A,F(3),A,F(1),A;
  PUT TOCHAR(255),'DII',92,'BHKW1 BEREIT             ',1,TOCHAR(254) TO A1 BY A,A,F(3),A,F(1),A;
  DIWERT(92)='1'B;
! PUT TOCHAR(255),'DII',93,'BHKW2 BETRIEB            ',0,TOCHAR(254) TO A1 BY A,A,F(3),A,F(1),A;
! PUT TOCHAR(255),'DII',94,'BHKW2 STOERUNG           ',0,TOCHAR(254) TO A1 BY A,A,F(3),A,F(1),A;
! PUT TOCHAR(255),'DII',95,'BHKW2 BEREIT             ',1,TOCHAR(254) TO A1 BY A,A,F(3),A,F(1),A;
  DIWERT(95)='1'B;
  AFTER 0.010 SEC RESUME;
  PUT 'DIEND' TO A1 BY A;
  AFTER 0.010 SEC RESUME;

  FOR I TO N_RELPLT*8 REPEAT
    PUT TOCHAR(255),'DOI',I,DO_NAME(I),TOCHAR(254) TO A1 BY A,A,F(3),A,A,A;
    AFTER 0.010 SEC RESUME;
  END;  
  AFTER 0.010 SEC RESUME;
  PUT 'DOEND' TO A1 BY A;
  AFTER 0.010 SEC RESUME;

  FOR I TO N_ANALOG REPEAT
    PUT TOCHAR(255),'AOI',I,AP_NAME(I),TOCHAR(254) TO A1 BY A,A,F(3),A,A,A;
    AFTER 0.010 SEC RESUME;
  END;  
  AFTER 0.010 SEC RESUME;
  PUT 'AOEND' TO A1 BY A;

  AFTER 2 SEC ACTIVATE SENDTEIL;

END;


CORE: TASK PRIO 14;
  DCL COREFRQ         FIXED(31);
  DCL p_COREFRQ   REF FIXED(31) ;

  NEWDAT='/TY';
  OPEN A1 BY IDF(NEWDAT);


  p_COREFRQ = NIL ; CALL REFADD( p_COREFRQ, TOFIXED('00005300'B4)//4(31) ) ;

  COREFRQ=p_COREFRQ;
 
  PUT 'Frequenz=',COREFRQ,'kHz' TO A1 BY A,F(8),A,SKIP;
  IF COREFRQ==56000(31) THEN
    PUT 'IEP CORE !!!!' TO A1 BY A,SKIP;
  FIN;
  IF COREFRQ==40000(31) THEN
    PUT 'PHYTEK       ' TO A1 BY A,SKIP;
  FIN;
...
SENDCANFIXED: PROC((NR,ID,LEN) FIXED, (F1,F2,F3,F4) FIXED(15), VERSUCHE FIXED);
  DCL error           FIXED(15);
  DCL CH              FIXED(15);
...
!     FOR I TO 4 REPEAT
!       PWMAA(I)=ROUND((((AP_UHIGH(I+8)-AP_ULOW(I+8))*0.01*X_AAUS(I+8)+AP_ULOW(I+8))/10.0)*1000.0);
!     END; 
...
!     FOR I TO 4 REPEAT
!       PWMAA(I)=ROUND((((AP_UHIGH(I+12)-AP_ULOW(I+12))*0.01*X_AAUS(I+12)+AP_ULOW(I+12))/10.0)*1000.0);
!     END; 
...
!     FOR I TO 4 REPEAT
!       PWMAA(I)=ROUND((((AP_UHIGH(I+16)-AP_ULOW(I+16))*0.01*X_AAUS(I+16)+AP_ULOW(I+16))/10.0)*1000.0);
!     END; 
...
! PUT 'O A1.;FORM D /R0/C5DS10' TO RTOS BY A;
  ...
  AFTER 8 SEC ACTIVATE INITSIM;
  AFTER 10 SEC ACTIVATE SERTAST;
...
SERTAST: TASK PRIO 10;
  ...
  REPEAT
    GET CHAR1  FROM TAST1 BY A(1);
    IF TOFIXED(CHAR1)/=0 THEN
      !PUT TOFIXED(CHAR1) TO A1 BY F(4),SKIP;
      IF TOFIXED(CHAR1)==127 THEN
        !PUT 'SERTAST 127' TO A1 BY A,SKIP;
        GET CHAR1  FROM TERM BY A(1);
        IF TOFIXED(CHAR1)==126 THEN  /* Meldung ¸ber Ver‰nderung AI */
          !PUT 'SERTAST 126' TO A1 BY A,SKIP;
          GET INDEX FROM TERM BY F(3);
          !PUT 'SERT ',INDEX TO A1 BY A,F(5),SKIP;
          IF INDEX > 0 AND INDEX < 121 THEN
            GET F31 FROM TERM BY F(6);
            AIWERT(INDEX)=(F31-1000)*0.1;
            !PUT 'SERT ',AIWERT(INDEX) TO A1 BY A,F(6,1),SKIP;
          FIN;
        FIN;
        IF TOFIXED(CHAR1)==125 THEN  /* Meldung ¸ber Ver‰nderung DI */
          !PUT 'SERTAST 125' TO A1 BY A,SKIP;
          GET INDEX FROM TERM BY F(3);
          !PUT 'SERT ',INDEX TO A1 BY A,F(5),SKIP;
          IF INDEX > 0 AND INDEX < 121 THEN
            GET F31 FROM TERM BY F(1);
            IF F31 > 0 THEN
              DIWERT(INDEX)='1'B;
            ELSE
              DIWERT(INDEX)='0'B;
...
STICK: PROC GLOBAL;
  ...
  ! ID_XR3=1393;      ! H_Taste
  ! ID_ZFERN3=1394;   ! H_Fernbed_nr
  ! ID_XGEHEIM3=1395; ! H_GEHEIM
    ID_XR3=  90;      ! H_Taste
...
/*********************************************************************/
DIN: TASK PRIO 4;    /* DigitaleingÑnge auslesen */
  ...
  REPEAT
    ...
    B_BL(1)=DIWERT(90);
    B_BSTOER(1)=DIWERT(91);
    B_BBEREIT(1)=DIWERT(92);
    IF B_BL(1) THEN
      PE_BIST(1)=11.0;
      PT_BIST(1)=25.0;
    ELSE
...
!   B_BL(2)=DIWERT(93);
!   B_BSTOER(2)=DIWERT(94);
!   B_BBEREIT(2)=DIWERT(95);
!   IF B_BL(2) AND B_KRAFTWERK THEN
!     PE_BIST(2)=11.0;
!     PT_BIST(2)=25.0;
!   ELSE
!     PE_BIST(2)=0.0;
...
!   X_AEINEXT(50)=AIWERT(35);
!   X_AEINEXT(51)=AIWERT(36);
!   X_AEINEXT(52)=AIWERT(37);
!   X_AEINEXT( 8)=AIWERT(38);
!   X_AEINEXT( 9)=AIWERT(39);

!   B_BL(2)=DIWERT(52);
!   B_BSTOER(2)=DIWERT(53);
!   B_BBEREIT(2)=DIWERT(54);
!   IF B_BL(2) AND B_KRAFTWERK THEN
!     PE_BIST(2)=11.0;
!     PT_BIST(2)=25.0;
!   ELSE
...
/****************************************************************/
/* Minieditor fuer eine Displayseite...                         */
/****************************************************************/
EDEND: TASK PRIO 30;
  PUT 'TERMINATE EDINFO' TO RTOS;
END;

EDPOS: PROC ((X,Y) FIXED IDENT);      
  IF X > 80 THEN X=80; FIN;
  IF X <  1 THEN X= 1; FIN;
  IF Y > 25 THEN Y=25; FIN;
  IF Y <  1 THEN Y= 1; FIN;

  PUT TOCHAR(27),'Y',TOCHAR(31+1),TOCHAR(31+68) TO TERM;
  PUT Y TO TERM BY F(3);
  PUT TOCHAR(27),'Y',TOCHAR(31+1),TOCHAR(31+74) TO TERM;
  PUT X TO TERM BY F(3);
  PUT TOCHAR(27),'Y',TOCHAR(31+Y+1),TOCHAR(31+X) TO TERM;
END;

EDINFO: TASK PRIO 30;

  DCL CH8025(80,26)  CHAR(1);
  DCL CHAR1          CHAR(1);
  DCL BEDIT          BIT(1);
  DCL BESC           BIT(1);
  DCL ZEI            FIXED;
  ...
  AFTER 60 MIN ACTIVATE EDEND;
  TERMINATE SERTAST;

  PUT TO INFO  BY LIST;
  OPEN INFO  BY IDF('INFO'),ANY;

  CALL READ(INFO,CH8025);

  PUT TOCHAR(27),'.','3' TO TERM;

  IF CH8025(1,26) /= 'N' OR CH8025(2,26) /= 'e' OR CH8025(3,26) /= 'u' THEN
    FOR I TO 26 REPEAT
      FOR K TO 80 REPEAT
         CH8025(K,I)=' ';
      END;
  ...
  /* D_CLR  */
  PUT TOCHAR(27),'T',         ! TextModus aktivieren
      TOCHAR(27),'I',         ! AttributPlane aktivieren
      TOCHAR(27),'@',         ! Plane lîschen
      TOCHAR(27),'N',         ! TextPlane aktivieren
      TOCHAR(27),'@',         ! Plane lîschen
      TOCHAR(27),'H',         ! Cursor Home
      TOCHAR(27),'V'          ! no auto wrap
  TO TERM;

  ZEI=1;
  SPA=1;
  PUT 'Info:  ',IDSTRING ,' Ende:  ESC,ESC   Z:',ZEI,' S:',SPA 
    TO TERM BY A,A,A,F(3),A,F(3),SKIP;

  FOR I TO 25 REPEAT
    FOR K TO 80 REPEAT
      IF TOFIXED(CH8025(K,I)) < 32 OR TOFIXED(CH8025(K,I)) > 127 THEN
        CH8025(K,I)=' ';
      FIN;
      PUT CH8025(K,I) TO TERM BY A(1);
    END;
    PUT TO TERM BY SKIP;
  END;  
  CALL EDPOS(SPA,ZEI);
  ...
  WHILE BEDIT REPEAT
    ...
    IF TOFIXED(CHAR1)/=0 THEN
      IF TOFIXED(CHAR1) > 31 THEN
        CH8025(SPA,ZEI)=CHAR1;
        PUT CHAR1 TO TERM BY A;
        SPA=SPA+1;
        CALL EDPOS(SPA,ZEI);
        BESC='0'B;
      ELSE
        IF TOFIXED(CHAR1)== 8 THEN  /* links */
          SPA=SPA-1;
          CALL EDPOS(SPA,ZEI);
        FIN;
        IF TOFIXED(CHAR1)==12 THEN  /* rechts */
          SPA=SPA+1;
          CALL EDPOS(SPA,ZEI);
        FIN;
        IF TOFIXED(CHAR1)==11 THEN  /* oben */
          ZEI=ZEI-1;
          CALL EDPOS(SPA,ZEI);
        FIN;
        IF TOFIXED(CHAR1)==10 THEN  /* unten */
          ZEI=ZEI+1;
          CALL EDPOS(SPA,ZEI);
        FIN;
        IF TOFIXED(CHAR1)==27 THEN  /* ESC */
          IF BESC THEN  BEDIT='0'B;  FIN;
          BESC='1'B;
        ELSE
          BESC='0'B;
        FIN;
        IF TOFIXED(CHAR1)==13 THEN  /* CR */
          ZEI=ZEI+1;
          SPA=1;
          CALL EDPOS(SPA,ZEI);
        FIN;
  ...
  PUT TO INFO  BY LIST;
  OPEN INFO  BY IDF('INFO');
  CALL REWIND(INFO);
  CALL WRITE (INFO,CH8025);
  CALL SYNC(INFO);
  CLOSE INFO;
  ...
  PUT TOCHAR(27),'T',         ! TextModus aktivieren
      ...
      TOCHAR(27),'V'          ! no auto wrap
  TO TERM;
  PUT 'EDINFO beendet' TO TERM BY A;
  ACTIVATE SERTAST;
