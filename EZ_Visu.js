...
window.addEventListener('load', function () {
  ...
  window.reloadLiveDataIntervalId = setInterval(getLiveData, 5000, window.visuLiveData, window.projectNo);
...
function getLiveData(visuLiveData, projectNo) {
  ...
  if (visuLiveData) {
    //console.log(visuLiveData);
    const divVisu = document.querySelector(`.divVisu`);
    Object.entries(visuLiveData).forEach(([key, value]) => {
...
function fetchData(projectNo) {
  ...
  return (projectNo === `test`) ? reformatLiveData(JSON.parse(VISU_LIVE_DATA).Items) : undefined;
}

function reformatLiveData(liveData) {
  const reformattedLiveData = {};
  liveData.filter(signal => signal.Bezeichnung.trim() !== `HK` && signal.Bezeichnung.trim() !== `BHK` && signal.Bezeichnung.trim() !== `KES` && signal.Bezeichnung.trim() !== `WWL`).forEach(signal => {
    const {Bezeichnung, Kanal, Wert, sWert} = signal;
    reformattedLiveData[`${Bezeichnung}${Kanal}`] = (sWert) ? sWert.trim() : Wert;
  });
